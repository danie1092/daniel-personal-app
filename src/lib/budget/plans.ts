 ...
 * 기본값(BUDGET_TARGETS)에 오버라이드를 얹어 그 달의 "유효 변동예산"을 만든다.
 * 특별예산(기본 카테고리에 없는 이름, 예: '부모님 생신')은 그대로 별도 줄이 된다.
...
/** 기본 변동예산 + 오버라이드 병합 (고정비 키는 무시). 순수 함수. */
export function effectiveVariableTargets(
  overrides: BudgetOverride[]
): Record<string, number> {
  const targets: Record<string, number> = { ...(BUDGET_TARGETS as Record<string, number>) };
...
/** 그 달의 유효 변동예산 합 */
export function effectiveVariableBudget(overrides: BudgetOverride[]): number {
  return Object.values(effectiveVariableTargets(overrides)).reduce((s, v) => s + v, 0);
}
