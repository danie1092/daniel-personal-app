...
describe("paceComment", () => {
  ...
  test("예산 초과면 페이스 무관하게 최고 경고", () => {
    expect(paceComment(1_300_000, 1_278_000, 0.99)).toBe("거지가 되고싶냐?");
  });
  test("페이스보다 한참 앞서면(+15%p) 강한 경고", () => {
    // 진행 50%인데 70% 씀
    expect(paceComment(700_000, 1_000_000, 0.5)).toBe("미쳤냐?");
  });
  test("페이스보다 조금 앞서면(+5~15%p) 중간 경고", () => {
    expect(paceComment(580_000, 1_000_000, 0.5)).toBe("좀만 더 아끼자...!");
  });
  test("페이스대로면(±5%p) 순한 잔소리", () => {
    expect(paceComment(500_000, 1_000_000, 0.5)).toBe("슬슬 조심해야겠는걸");
  });
  test("페이스보다 아끼는 중이면 칭찬 — 월말에 80% 썼어도 잔소리 X", () => {
    expect(paceComment(800_000, 1_000_000, 0.9)).toBe("이번달도 아껴쓰자!");
  });
});
