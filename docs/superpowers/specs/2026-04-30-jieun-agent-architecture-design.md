...
2. **하루 리듬 잡아주기**: 점심·퇴근·귀가·회고 4개 시간대에 봇이 가벼운 노크 / 브리핑 / 회고 대화를 진행한다.
3. **자율 데이터 기록**: 사용자가 봇과 대화하면 봇이 가계부·메모·루틴 등에 직접 INSERT. 입력 마찰 0.
4. **잠재 관찰**: 6시간마다 봇(=Claude)이 최근 데이터를 자체 판단으로 훑고 발화/침묵 결정.
5. **앱은 viewer로 살아남음**: 길게 보기, 일기 길게 쓰기, 봇이 잘못 기록한 것 사후 수정. 기존 기능 다 유지.
6. **추가 비용 0**: 사용자 기존 Claude Max 구독 안에서 모든 Claude 호출 처리.

## 비목표 (v1)

- **메뉴 추천**: 좋아하는 메뉴, 월간 주기, 생리주기 영향 — v2 별도 spec. v1은 봇이 점심 메뉴를 묵묵히 *기록*하는 단계까지만.
- **캘린더 쓰기**: v1은 Apple Calendar 읽기만 (브리핑 용도). 일정 추가/수정은 v2.
...
- **고도화된 페르소나 시스템**: 톤은 프롬프트 룰로만 관리. 페르소나 엔진/자체 학습 X.
...
### 캘린더
23. **Apple Calendar (개인) read-only**.
    - 분리 유지: Google Calendar(업무)는 v1에서 절대 안 봄. 봇 컨텍스트는 *개인 삶의 흐름*만.
    - 읽기 방식: 맥미니에서 `icalBuddy` CLI 또는 EventKit 기반 작은 헬퍼. v1은 `icalBuddy` 우선 (의존성 1개, 권한 단순).
...
26. **시그널 계산 위치**: 맥미니 봇 프로세스 내. Supabase에서 raw 데이터 read → JS로 집계 → `bot_signals`에 후보로 저장. Claude는 이 후보를 보고 *발화할지/어떻게 표현할지* 판단.
27. **임계값**: v1은 휴리스틱 (예: 카테고리 평균 대비 1.5배 이상 + 절대값 5만원 이상이면 후보). 데이터 쌓이면 v1.5에서 조정.

### 장애 / 실수 처리
28. **맥미니 다운**: 봇 침묵. 텔레그램 outage notification 없음 (침묵이 기본 철학에 부합).
29. **봇이 잘못 기록**: `/bot-log` 화면에서 다영이 직접 수정/삭제. 봇은 사후 수정 사실을 다음 회고 대화 때 알 수 있음 (`bot_writes`의 수정 이력 컨텍스트로).
30. **봇이 너무 시끄러움**: 다영이 `조용히` → 24시간 mute. 또한 봇이 *연속 발화 3회* (하루 안에 같은 종류 아님에도 3번 넘게 말 걸면) 자동 backoff.
31. **Claude 인증 만료**: 11번에 정의된 알림 + backoff.
32. **Telegram API 장애**: polling이라 자동 재시도. 5분 이상 끊기면 launchd가 프로세스 재시작 (StandardOutPath/StandardErrorPath 로그 + `KeepAlive`).

### 보안 / 프라이버시
33. **민감 정보 마스킹**: Claude 호출 시 카드번호 끝 4자리 등 민감 토큰은 `[CARD]` 같은 식으로 마스킹 후 전송. (현재 `budget_entries.memo`엔 merchant만 들어가서 큰 문제 없지만, raw SMS 본문은 절대 Claude로 보내지 않음.)
34. **Telegram chat_id whitelist**: 다영 1개. 그 외 chat_id의 메시지는 즉시 무시 + 로그.
35. **봇 토큰**: macOS Keychain 또는 mode 600 .env. 절대 git 커밋 X (이미 .gitignore 적용).
36. **Supabase RLS**: 기존 그대로. 봇은 사용자가 아니라 *서비스* 입장이므로 service_role key를 맥미니에 두고 사용 (기존 Phase 3 SMS와 동일 패턴).
37. **로그 위생**: 대화 로그(`bot_conversations`)는 Supabase에 RLS로 보호. 맥미니 로컬 파일 로그(`failed-claude.log` 등)는 mode 600 + 100KB 회전 + 5개 보존 (Phase 3와 동일).
...
-- 4. 봇 자율 기록 추적 (사후 수정 위해)
CREATE TABLE IF NOT EXISTS bot_writes (
  id           uuid          PRIMARY KEY DEFAULT gen_random_uuid(),
  target_table text          NOT NULL,     -- 'budget_entries' | 'memo_entries' | ...
  target_id    uuid          NOT NULL,     -- INSERT한 row의 id
...
-- 5. RLS — 단일 사용자라 service_role 키 기반. 일반 RLS는 폐쇄.
...
-- 단, /bot-log 페이지에서 다영이 봐야 하므로 authenticated에 SELECT만 풀어준다.
...
│   │   └── tools.ts           # read_db / write_db / read_calendar 도구 정의
│   ├── memory/
...
implementation plan은 이 spec 컨펌 후 별도 문서로 분리한다. 큰 단계만:

1. **인프라 부트**: Mac mini에서 echo bot 항상 켜기 (Telegram polling만, Claude 없음). launchd 작동 확인.
2. **Claude 어댑터 + 페르소나 prompt**: Agent SDK 또는 CLI 호출. `다영아 안녕` 같은 단일 응답 가능 상태.
3. **메모리 모델**: `bot_conversations` 저장 + 메모리 로더. 어제 대화 기억하는 봇 상태.
4. **시간 트리거 1개부터**: 점심 노크 12:30. 아주 단순하게.
5. **데이터 쓰기 권한 + 도구**: 봇이 가계부 INSERT 할 수 있게. `bot_writes` 추적.
6. **앱 `/bot-log` 화면**: 봇 기록 7일치 보고 수정/삭제.
7. **남은 시간 트리거 4개**: 아침 브리핑, 퇴근, 귀가, 회고.
8. **이벤트 트리거**: SMS 도착 → 시그널 계산 → 발화 후보.
9. **캘린더 연동**: icalBuddy로 아침/퇴근직전 브리핑.
10. **잠재 관찰 (6h)**: 가장 마지막. 다른 게 다 안정된 후에만.
11. **회고 대화 모드**: 평소 응답보다 길게 (10문장), 좋았던 점/아쉬운 점/내일 한 가지 구조.
12. **수동 mute / 도배 방지 강화**.
